...
const makeHtml = (jsonData, day) => {
  ...
  for (index = 0; index < jsonData.length; index++) {
    ...
    menu += `<div class="data" onClick="order(${JSON.stringify(info)
      .split('"')
      .join("&quot;")})"
...
function order(data) {
  ...
  if (info[5] == false) {
    ...
    menu.setAttribute("date-date", info[6]);
...
function callData() {
  ...
  for (index = 0; index < orders.length; index++) {
    json.push({
      reCode: orders[index].dataset.recode,
      mCode: orders[index].dataset.menucode,
      rDate: orders[index].dataset.date,
      quantity: orders[index].childNodes[1].value,
    });
  }
